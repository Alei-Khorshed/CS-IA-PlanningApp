...
# Function to load planning data and display information 
def load_planning_data(conn : sql.Connection):
...
def display_pending_tasks(conn : sql.Connection):
    ...
    df_task = pd.read_sql("SELECT task_id, Task.title , Subject.title as subject, deadline, difficulty, status FROM Task INNER JOIN subject ON task.subject_id = subject.subject_id Where status='PENDING' AND user_id = ?", conn , params=[st.session_state.gCurrentUser])
...
try:
    ...
    # Load planning data
    load_planning_data(st.session_state.gDBConnection)
    ...
    display_pending_tasks(st.session_state.gDBConnection)

    st.divider()
    # display completed tasks
    display_completed_tasks(st.session_state.gDBConnection)
